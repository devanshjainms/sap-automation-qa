...
@mcp.tool(
    ...
    description=(
        ...
        "test_group=CentralServicesHighAvailability for SCS systems — "
        "NOT ConfigurationChecks. ConfigurationChecks validates OS-level "
        "settings (disks, packages, kernel parameters) across node tiers. "
        "Recognized test_ids for DatabaseHighAvailability (from input-api.yaml): "
        "ha-config, ha-config-offline, azure-lb, resource-migration, "
        "primary-node-crash, primary-node-kill, primary-crash-index, "
        "primary-echo-b, primary-worker-node-kill, primary-worker-echo-b, "
        "primary-worker-crash-index, secondary-node-kill, secondary-crash-index, "
        "secondary-echo-b, secondary-worker-node-kill, secondary-worker-echo-b, "
        "secondary-worker-crash-index, block-network, secondary-block-network, "
        "block-hana-shared, fs-freeze, sbd-fencing. "
        "Recognized test_ids for CentralServicesHighAvailability: "
        "ha-config, ha-config-offline, azure-lb, sapcontrol-config, "
        ...
        "block-network. "
        "Recognized test_ids for AzureBackupDatabase: "
        "backup-setup-verification, restore-to-db, restore-to-filesystem, "
        "recover-db-commands, restore-cross-vm. "
        "ConfigurationChecks does not use the same test_id filter system; "
        "pass a descriptive test_ids value (e.g. ['all']) to satisfy the "
        "required parameter. "
        "Returns a job_id — poll with get_job_status until is_terminal=true, "
        "then call get_job_results for the pass/fail summary."
    ),
