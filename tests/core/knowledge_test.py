...
class TestKnowledgeGap:
    """Unit tests for KnowledgeGap model."""

    def test_create(self) -> None:
        """Verify knowledge gap creation."""
        gap = KnowledgeGap(
            id="GAP-001",
            description="No rule for HANA nameserver trace pattern",
            session_id="sess-1",
        )
        assert gap.resolved is False

    def test_resolution(self) -> None:
        """Verify gap can be marked resolved."""
        gap = KnowledgeGap(
            id="GAP-002",
            description="Missing DB2 HADR rule",
            session_id="sess-2",
            resolved=True,
        )
...
class TestExperienceEntry:
    """Unit tests for ExperienceEntry model."""

    def test_create(self) -> None:
        """Verify experience entry creation matching Section 7.2.3."""
        ...
        assert entry.session_id == "session-2025-0110-001"
        assert entry.system_id == "PRD-HANA-01"
        assert entry.trigger == "ha_failover_test"
        assert entry.duration_seconds == 342
        assert len(entry.patterns_matched) == 2
        assert entry.rules_fired == 47
        assert entry.rules_failed == 3
        assert entry.root_cause_found is True
        assert entry.resolution_applied is True
        assert entry.operator_feedback == "correct"
        assert entry.knowledge_gaps == []

    def test_defaults(self) -> None:
        """Verify experience entry defaults."""
        entry = ExperienceEntry(session_id="sess-1")
        assert entry.system_id == ""
        assert entry.trigger == ""
        assert entry.duration_seconds == 0.0
        assert entry.patterns_matched == []
        assert entry.rules_fired == 0
        assert entry.rules_failed == 0
        assert entry.root_cause_found is False
        assert entry.resolution_applied is False
        assert entry.operator_feedback is None
        assert entry.knowledge_gaps == []
