...
class TestSshCredentialCache:
    ...
    def test_provision_delegates_to_provider(self) -> None:
    ...
    def test_cache_hit_avoids_reprovision(self) -> None:
        provider = _make_provider()
        cache = SshCredentialCache(provider)
    ...
    def test_different_workspaces_cached_separately(self) -> None:
        provider = _make_provider()
        cache = SshCredentialCache(provider)
    ...
    def test_expired_entry_reprovisions(self) -> None:
        provider = _make_provider()
    ...
    def test_expired_entry_cleanup_called(self) -> None:
        cred1 = _make_credential("WS1")
        cred1.cleanup = MagicMock()
        provider = _make_provider(side_effect=[cred1, _make_credential("WS1")])
        cache = SshCredentialCache(provider, ttl_seconds=1)
    ...
    def test_clear_removes_all(self) -> None:
        ...
        cred2 = cache.provision("WS2", {})
        cred1.cleanup = MagicMock()
        cred2.cleanup = MagicMock()

        # Re-cache with mocked cleanup
    ...
    def test_provision_returns_none_on_failure(self) -> None:
        provider = _make_provider(side_effect=[None])
        cache = SshCredentialCache(provider)
