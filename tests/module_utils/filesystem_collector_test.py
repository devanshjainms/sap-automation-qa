...
class TestClassifyHanaStorageTypes:
    """Test mount-specific HANA storage classification."""

    def test_classifies_managed_hana_storage(self, collector):
        """Test correlated managed disks produce a managed HANA I/O profile."""
        filesystems = [
            {"target": "/hana/data", "azure_disk_names": ["data0", "data1"]},
            {"target": "/hana/log", "azure_disk_names": ["log0"]},
            {"target": "/hana/shared", "azure_disk_names": ["shared0"]},
        ]
        azure_disk_data = [
            {"name": disk_name, "sku": "Premium_LRS"}
            for disk_name in ["data0", "data1", "log0", "shared0"]
        ]

        result = collector.classify_hana_storage_types(filesystems, azure_disk_data)

        assert result["hana_data_storage_type"] == ["Premium_LRS"]
        assert result["hana_log_storage_type"] == ["Premium_LRS"]
        assert result["hana_shared_storage_type"] == ["Premium_LRS"]
        assert result["storage_type"] == ["Premium_LRS"]

    def test_classifies_mixed_hana_storage(self, collector):
        """Test managed and NFS mounts are classified independently."""
        filesystems = [
            {"target": "/hana/data", "azure_disk_names": ["data0", "data1"]},
            {"target": "/hana/log", "azure_disk_names": ["log0"]},
            {"target": "/hana/shared", "nfs_type": "ANF"},
        ]
        azure_disk_data = [
            {"name": "data0", "sku": "Premium_LRS"},
            {"name": "data1", "sku": "Premium_LRS"},
            {"name": "log0", "sku": "PremiumV2_LRS"},
        ]

        result = collector.classify_hana_storage_types(filesystems, azure_disk_data)

        assert result["hana_data_storage_type"] == ["Premium_LRS"]
        assert result["hana_log_storage_type"] == ["PremiumV2_LRS"]
        assert result["hana_shared_storage_type"] == ["ANF"]
        assert result["storage_type"] == ["ANF"]

    def test_classifies_afs_and_leaves_unmapped_mounts_unknown(self, collector):
        """Test AFS correlation and unknown managed mounts do not invent a type."""
        filesystems = [
            {"target": "/hana/data", "azure_disk_names": ["missing-disk"]},
            {"target": "/hana/log", "nfs_type": "AFS"},
        ]

        result = collector.classify_hana_storage_types(filesystems, [])

        assert result["hana_data_storage_type"] == []
        assert result["hana_log_storage_type"] == ["AFS"]
        assert result["hana_shared_storage_type"] == []
        assert result["storage_type"] == ["AFS"]
