...
class TestIsCheckApplicable:
    ...
    def test_hana_checks_use_mount_specific_storage(self, config_module):
        """Test mixed HANA storage selects mount and host profiles independently."""
        hana_checks = (
        ...
        config_module.set_context(
            {
                ...
                "hana_shared_storage_type": ["ANF"],
            }
        )

        assert config_module.is_check_applicable(checks["DB-HANA-0041"]) is True
        assert config_module.is_check_applicable(checks["DB-HANA-0043"]) is True
        assert config_module.is_check_applicable(checks["DB-HANA-0050"]) is False
        assert config_module.is_check_applicable(checks["DB-HANA-0051"]) is False
        assert config_module.is_check_applicable(checks["DB-HANA-0057"]) is True
