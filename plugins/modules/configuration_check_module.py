...
DOCUMENTATION = r"""
---
module: configuration_check_module
short_description: Validate SAP workload configuration on Azure
version_added: "1.0.0"
description:
  - Runs configuration checks against SAP workloads deployed on Azure.
  - Validates system settings, cluster configuration, and Azure resource alignment.
  - Supports parallel execution of checks via ThreadPoolExecutor.
  - Collects data from commands, Azure APIs, and the local filesystem.
options:
  check_file_content:
    description:
      - YAML string defining the configuration checks to execute.
    type: str
    required: true
  context:
    description:
      - Dictionary of system context variables (SID, instance number, OS family, etc.).
    type: dict
    required: true
  filter_tags:
    description:
      - List of tags to filter which checks to run.
    type: list
    elements: str
    required: false
    default: null
  filter_categories:
    description:
      - List of categories to filter which checks to run.
    type: list
    elements: str
    required: false
    default: null
  parallel_execution:
    description:
      - Whether to execute checks in parallel using a thread pool.
    type: bool
    required: false
    default: false
  max_workers:
    description:
      - Maximum number of threads when parallel_execution is enabled.
    type: int
    required: false
    default: 3
  enable_retry:
    description:
      - Whether to retry failed checks.
    type: bool
    required: false
    default: false
  workspace_directory:
    description:
      - Path to the workspace directory for storing results and artifacts.
    type: str
    required: true
  hostname:
    description:
      - Hostname of the system being checked. Auto-detected if not provided.
    type: str
    required: false
    default: null
  test_group_invocation_id:
    description:
      - Unique identifier for this test group invocation.
    type: str
    required: true
  test_group_name:
    description:
      - Name of the test group being executed.
    type: str
    required: true
  azure_resources:
    description:
      - Dictionary of Azure resource information for validation.
    type: dict
    required: false
    default: {}
author:
...
RETURN = r"""
results:
  description: List of check results with status, severity, and details.
  type: list
  returned: always
test_group_invocation_id:
  description: The invocation ID for this test group run.
  type: str
  returned: always
"""
