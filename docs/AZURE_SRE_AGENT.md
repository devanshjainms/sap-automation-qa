...
STAF bridges this gap by exposing **25 MCP tools** that give Azure SRE Agent
the ability to:
...
4. Select tools — see [tool-selection-guide.md](sre-agent/tool-selection-guide.md)
   for tier recommendations (12 essential / 20 standard / 25 full).
...
| Tool | Description |
|------|-------------|
| `run_analysis` | Analyze evidence against 400+ SAP rules |
| `get_triage_report` | Retrieve formatted triage report |
...
| Tool | Description |
|------|-------------|
| `record_investigation_outcome` | Record operator feedback for learning |
